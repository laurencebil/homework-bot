...
async def addhw(ctx, subject: str, *, due: str):
    ...
    try:
        ...
        await ctx.send(
            f"✅ Homework added: **{subject}** (due {discord.utils.format_dt(due_dt, 'F')}, {discord.utils.format_dt(due_dt, 'R')})"
        )
    except Exception as e:
        await ctx.send("⚠️ Couldn't read the date/time! Try `7pm` or `10/5/25 7pm`")
        print(e)
